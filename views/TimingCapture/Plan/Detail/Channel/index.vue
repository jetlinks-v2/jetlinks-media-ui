    ...
    <div>
        <a-spin :spinning="spinning">
            <div class="channelControl">
                <div class="bind">{{ $t('Channel.index.855184-0') }}{{ bindCount }}</div>
                <a-space v-if="editType">
                    <a-button @click="clearBind">{{ $t('Channel.index.855184-1') }}</a-button>
                    <a-button @click="showBind">{{ $t('Channel.index.855184-2') }}</a-button>
                </a-space>
                <div v-else>
                    <j-permission-button
                        type="link"
            ...
            <div class="bound">
                <div class="bound_device">
                    <div v-if="showBody">{{ $t('Channel.index.855184-3') }}</div>
                    <ChannelTree
                        ref="treeRef"
                        :height="700"
                        :id="route.params.id"
                ...
                <div v-if="showBody" class="bound_channel">
                    <div style="padding: 12px 24px 0; display: flex">
                        <div class="catalogue">{{ $t('Channel.index.855184-4') }}</div>
                        <a-breadcrumb>
                            ...
                            <a-space :size="16">
                                <template v-if="editType">
                                    <j-permission-button
                                        ...
                                        style="padding: 0px"
                                        key="play"
                                        :tooltip="{ title: $t('Channel.index.855184-5') }"
                                        @click="
                                            () => {
                                                onPlay(slotProps);
                                            }
                                    ...
                                    <j-permission-button
                                        ...
                                        key="unbind"
                                        style="padding: 0px"
                                        :tooltip="{ title: $t('Channel.index.855184-6') }"
                                        :popConfirm="{
                                            title: $t('Channel.index.855184-7'),
                                            okText: $t('Channel.index.855184-8'),
                                            cancelText: $t('Channel.index.855184-9'),
                                            onConfirm: () => {
                                                unBind(slotProps);
                                            },
                                        }"
                                ...
                                <template v-else>
                                    <j-permission-button
                                        ...
                                        key="picture"
                                        style="padding: 0px"
                                        :tooltip="{ title: $t('Channel.index.855184-10') }"
                                        @click="
                                            () => {
                                                onPicture(slotProps);
                                            }
                                    ...
                                    <j-permission-button
                                        ...
                                        style="padding: 0px"
                                        key="logs"
                                        :tooltip="{ title: $t('Channel.index.855184-11') }"
                                        @click="
                                            () => {
                                                onLogs(slotProps);
                                            }
            ...
            <div v-if="showBody && editType">
                <a-button
                    ...
                    :loading="saveLoading"
                    @click="saveChannel"
                    >{{ $t('Channel.index.855184-12') }}</a-button
                >
            </div>
            <div v-if="!showBody" class="video-unbind-tip">{{ $t('Channel.index.855184-13') }}</div>
        </a-spin>
...
const columns = [
    ...
    {
        title: $t('Channel.index.855184-14'),
        dataIndex: 'name',
        key: 'name',
        ellipsis: true,
        search: {
    ...
    {
        title: $t('Channel.index.855184-15'),
        dataIndex: 'manufacturer',
        key: 'manufacturer',
        ellipsis: true,
    },
    {
        title: $t('Channel.index.855184-16'),
        dataIndex: 'address',
        ellipsis: true,
        key: 'address',
        search: {
    ...
    {
        title: $t('Channel.index.855184-17'),
        dataIndex: 'status',
        key: 'status',
        scopedSlots: true,
        width: 150,
        search: {
            type: 'select',
            options: [
                { label: $t('Channel.index.855184-18'), value: 'online' },
                { label: $t('Channel.index.855184-19'), value: 'offline' },
            ],
        },
    },
    {
        title: $t('Channel.index.855184-20'),
        key: 'action',
        width: 100,
        scopedSlots: true,
    },
...
const clearBind = () => {
    Modal.confirm({
        title: $t('Channel.index.855184-21'),
        onOk() {
            run(route.params.id);
...
const saveChannel = async () => {
    ...
    await getBindTotal();
    onlyMessage($t('Channel.index.855184-22'));
