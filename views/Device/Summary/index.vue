<template>
    ...
    >
        <template #title>
            ...
            <div class="deviceId">
                <span> {{ $t('Summary.index.534986-0') }} </span>
                <a @click="jumpDetail">{{ deviceData?.id }}</a>
            </div>
        </template>
        <div>
            <a-descriptions bordered :column="1">
                <a-descriptions-item :label="$t('Summary.index.534986-1')">{{
                    PROVIDER_OPTIONS.find(
                        (i) => i.value === deviceData?.provider,
                    )?.label
                }}</a-descriptions-item>
                <a-descriptions-item :label="$t('Summary.index.534986-2')">{{
                    deviceData?.productName
                }}</a-descriptions-item>
                <a-descriptions-item
                    v-if="deviceData?.provider === 'onvif'"
                    :label="$t('Summary.index.534986-3')"
                    >{{ deviceData?.others?.onvifUrl }}</a-descriptions-item
                >
                <a-descriptions-item
                    v-if="deviceData?.provider === 'onvif'"
                    :label="$t('Summary.index.534986-4')"
                    >{{
                ...
                <a-descriptions-item
                    ...
                    "
                    :label="$t('Summary.index.534986-5')"
                    ><div class="password">
                        ...
                        ></AIcon></div
                ></a-descriptions-item>
                <a-descriptions-item :label="$t('Summary.index.534986-6')">{{
